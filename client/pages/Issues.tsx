    ...
    return allIssues.filter(
      (i) =>
        ...
        (q.trim() === "" ||
          ...
          i.location.address?.toLowerCase().includes(q.toLowerCase())),
    );
  }, [status, category, q]);
                ...
                <TableBody>
                  {issues.map((i) => (
                    <TableRow key={i.id}>
                      <TableCell>
                        <img
                          src={i.photoUrl || "/placeholder.svg"}
                          alt=""
                      ...
                      <TableCell>
                        <span
                          className={`inline-flex items-center gap-2 text-xs font-medium px-2 py-1 rounded-full border ${statusClass(i.status)}`}
                        >
                          {i.status.replace("_", " ")}
                        </span>
                      </TableCell>
                      <TableCell>
                        {staff.find((s) => s.id === i.assignedTo)?.name || "—"}
                      </TableCell>
                      <TableCell>
...
function statusClass(s: IssueStatus) {
  ...
  if (s === "RESOLVED")
    return "border-status-success/40 text-status-success bg-status-success/10";
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm">
                  Reporter: {issue.reporter?.name} · {issue.reporter?.phone}
                </div>
                <div className="text-sm">
